        ...
        try:
            if can_predict:
                ...
                if delta_days >= 0 and delta_days < 7:
                   ...
                   prev_weather = get_historical_weather_data(dep_cst_time,input.departure_airport(),input.arrival_airport())
                   if prev_weather is not None:
                    for key in prev_weather.keys():
                        flight_data[key] = prev_weather[key]
